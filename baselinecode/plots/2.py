...
def run_test(L: int, M: int, N: int) -> list[float]:
    ...
    std_args = subprocess.run(PROGRAM_RUN.format(max(L, 1), max(M, 1), max(N, 1), SEED, MODE, BLOCK_SIZE, THREADS), shell=True, capture_output=True, text=True)
...
if __name__ == "__main__":
    ...
    axis.plot(x, y, marker='o', linewidth=2, markersize=5, label="Custom")
